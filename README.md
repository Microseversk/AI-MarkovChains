...
## Работа с диалоговыми шутками
В репозитории появился корпус `jokes_with_dialogs.txt` с анекдотами, записанными в формате «- реплика».
Чтобы сохранить структуру диалога:

1. Передайте файл через `--dataset jokes_with_dialogs.txt`.
2. Добавьте `--preserve-linebreaks`, чтобы переносы строк превратились в отдельные токены и не склеивались в одну строку.
3. Оставьте `--split-sentences` выключенным (значение по умолчанию), если хотите, чтобы модель видела весь диалог целиком.

Пример команды:

```bash
python markov_jokes.py --dataset jokes_with_dialogs.txt --preserve-linebreaks --count 3 --max-length 80
```
...
# Диалоговый корпус с сохранением переносов строк
python markov_jokes.py --dataset jokes_with_dialogs.txt --preserve-linebreaks --count 3
```
...
| `--preserve-linebreaks` | Превращает переносы строк в отдельные токены. | Сохраняет формат «- реплика» и многострочные диалоги. |
...
## Uniqueness metric
- Печатается после каждой шутки как `(uniqueness: XX.X%)`.
- Считается по n-граммам длиной `--order`: берем все окна длиной `order` в тексте шутки и проверяем, каких не было в обучающем корпусе.
- Формула: `uniqueness = (количество новых n-грамм / всего n-грамм) * 100`. Чем выше процент, тем больше фрагментов, которых не было в датасете.
- Если токенов меньше `order`, метрика возвращает 100%; для пустой строки — 0%.
